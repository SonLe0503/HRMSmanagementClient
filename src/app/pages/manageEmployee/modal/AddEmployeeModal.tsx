...
const AddEmployeeModal = ({ open, onCancel, onSuccess }: AddEmployeeModalProps) => {
    ...
    const onFinish = (values: any) => {
        ...
        dispatch(createEmployee(payload))
            ...
            .catch((err: any) => {
                message.error(typeof err === "string" ? err : err?.message || "Thêm nhân viên thất bại!");
            });
    };
    ...
    return (
        <Modal title="Thêm nhân viên mới" open={open} onCancel={() => { form.resetFields(); onCancel(); }}
            ...
            <Form form={form} layout="vertical" onFinish={onFinish}>
                <Row gutter={16}>
                    <Col span={12}>
                        <Form.Item name="employeeCode" label="Mã nhân viên" rules={[{ required: true }, { max: 20 }]}>
                ...
                <Row gutter={16}>
                    <Col span={8}>
                        <Form.Item name="departmentId" label="Phòng ban">
                            <InputNumber style={{ width: "100%" }} min={1} placeholder="Dept ID" />
                        </Form.Item>
                    </Col>
                    <Col span={8}>
                        <Form.Item name="positionId" label="Chức vụ">
                            <InputNumber style={{ width: "100%" }} min={1} placeholder="Pos ID" />
                        </Form.Item>
                    </Col>
                    <Col span={8}>
                        <Form.Item name="managerId" label="Quản lý">
                            <InputNumber style={{ width: "100%" }} min={1} placeholder="Manager ID" />
                        </Form.Item>
                    </Col>
                </Row>
                <Row gutter={16}>
