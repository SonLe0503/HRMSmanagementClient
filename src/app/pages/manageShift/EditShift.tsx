...
import { Form, Input, InputNumber, TimePicker, Switch, message, Row, Col, Select, Card, Button, Typography, Space, Spin } from "antd";
import { ArrowLeftOutlined, SaveOutlined } from "@ant-design/icons";
import { useAppDispatch, useAppSelector } from "../../../store";
import { updateShift, fetchAllShifts, selectShifts, type IShift } from "../../../store/shiftSlide";
...
const EditShift = () => {
    ...
    useEffect(() => {
        if (shifts.length === 0) {
            dispatch(fetchAllShifts()).finally(() => setLoading(false));
        } else {
            setLoading(false);
        }
    ...
    useEffect(() => {
        if (!loading && id) {
            const foundShift = shifts.find(s => s.shiftId === Number(id));
            if (foundShift) {
                setShift(foundShift);
                form.setFieldsValue({
                    ...foundShift,
                    startTime: dayjs(`2000-01-01 ${foundShift.startTime}`),
                    endTime: dayjs(`2000-01-01 ${foundShift.endTime}`),
                });
            } else {
                message.error("Không tìm thấy ca làm việc!");
                navigate(URL.ManageShift);
            }
        }
    }, [id, shifts, loading, form, navigate]);
    ...
    if (loading || !shift) {
        return (
            <div style={{ display: "flex", justifyContent: "center", padding: "100px" }}>
                <Spin size="large" tip="Đang tải dữ liệu ca làm việc..." />
            </div>
        );
    }
    ...
    return (
        <div className="p-4" style={{ maxWidth: 800, margin: "0 auto" }}>
            <Space className="mb-4" align="center" style={{ marginBottom: 20 }}>
                ...
                <Title level={3} style={{ margin: 0 }}>Sửa Ca Làm Việc: {shift?.shiftName}</Title>
            </Space>
